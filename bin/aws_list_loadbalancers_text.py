...
# run as follows
# python3 aws_list_loadbalancers_text.py <region> <aws_profile>
# python3 aws_list_loadbalancers_text.py us-east-1 default
...
def describelbs():
  ...
  # all the data has been written, lets close the file
  print("Load Balancer info is written to ",textfilename)
  outfile.close()
    
if __name__ == "__main__":
  if len(sys.argv) < 3:
    print(" RUN IT AS: python3 aws_list_loadbalancers_text.py <region> <aws_profile> ")
    exit()
  region      = sys.argv[1]
  profile     = sys.argv[2]
